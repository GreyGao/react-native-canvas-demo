  ...
  componentWillMount() {
    this.panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        ...
        this._path.data.push(`${parseFloat(gestureState.x0 - this._offset.x).toFixed(2)},${parseFloat(gestureState.y0 - this._offset.y).toFixed(2)},${this._timeStamp}`)
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (this._path) {
          ...
          this._timeStamp = Date.now()
          this._path.data.push(`${parseFloat(gestureState.moveX - this._offset.x).toFixed(2)},${parseFloat(gestureState.moveY - this._offset.y).toFixed(2)},${this._timeStamp}`)
      ...
      onPanResponderRelease: (evt, gestureState) => {
        if (!this.props.touchEnabled) return
        if (this._path) {
          this.props.onStrokeEnd({ path: this._path, size: this._size, drawer: this.props.user, test: "1" })
